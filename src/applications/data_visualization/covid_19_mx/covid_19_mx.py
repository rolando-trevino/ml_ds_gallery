...
def write():
    ...
    if len(df) == 0:
        df = pd.read_csv(
            "https://raw.githubusercontent.com/rolando-trevino/ml_ds_gallery/main/data/gallery/data_visualization/covid_19_mx/per_state.csv"
        )
    ...
    if len(df) > 0:
        ...
        df_to_map = (
            df_to_map.reset_index()
            .groupby([pd.Grouper(level=0), pd.Grouper(key="date", freq="M")])
            .sum()
            .reset_index()
        )
        ...
        df_to_map["Date"] = [
            datetime.strftime(date(y, m, d), "%Y-%m-%d")
            for (y, m, d) in zip(
                df_to_map["year"], df_to_map["month"], df_to_map["day"]
            )
        ]
        # to commit
        fig = px.choropleth(
        ...
        fig = go.Figure(
            data=fig["frames"][-1]["data"], frames=fig["frames"], layout=fig.layout
        )
        ...
        if selected_state_int != 0:
            ...
            df_filtered.rename(
                columns={
                    "date": "Date",
                    "positive_symptoms": "Positive Cases (Symptoms)",
                    "positive_confirmed": "Positive Cases (Confirmed)",
                },
                inplace=True,
            )
        else:
        ...
        col1.write(
            f'<h2 style="text-align:center">Total Cases</h2>', unsafe_allow_html=True
        )
        col1.write(
            f"<p style=\"text-align:center\">{df_filtered['Positive Cases (Symptoms)'].values[-1]:,.0f}</p>",
            unsafe_allow_html=True,
        )

        col2.write(
            f'<h2 style="text-align:center">New Cases (7 days)</h2>',
            unsafe_allow_html=True,
        )
        col2.write(
            f"<p style=\"text-align:center\">+ {df_filtered['Positive Cases (Confirmed)'].diff(1).values[-7:].sum():,.0f}</p>",
            unsafe_allow_html=True,
        )
        ...
        df_filtered["Moving Average (7)"] = (
            df_filtered["New Cases"].rolling(window=7).mean()
        )
        fig = px.line(
            df_filtered,
            x="Date",
            y="Moving Average (7)",
            title="New Cases Moving Average (7 days)",
        )
        st.plotly_chart(fig, use_container_width=True)
